...
def track_changes(sender, instance: Defect, **kwargs):
    ...
    try: prev = Defect.objects.get(pk=instance.pk)
    except Defect.DoesNotExist: return
    ...
    for f in fields:
        if getattr(prev, f) != getattr(instance, f):
            History.objects.create(defect=instance, field=f, old_value=str(getattr(prev, f)),
                                   new_value=str(getattr(instance, f)), changed_by=getattr(instance,"_changed_by",None))
